...
-- Description:	Импорт данных из данных Profiler за день.
-- =============================================
exec [dbo].[XE_Import_Profile]
	 @Start		= '2024-06-18 00:00:00'
	,@Stop		= '2024-06-19 00:00:00'
	,@Source	= '[alidi backup].[dbo].[Prof_log_251_char]'
	,@Table		= 'XE_Profiler'
	,@Session	= null
	,@Option	= 'nodb list \only'
-- ============================================= */
--	Declare
	drop procedure if exists	[dbo].[XE_Import_Profile];
GO
	CREATE	PROCEDURE [dbo].[XE_Import_Profile]
...
,@Option		nvarchar(256)	= null	-- '\nodb list \only'
	AS BEGIN	-- [dbo].[Add_UpdateXE_ProfileImport]
...
if upper(' '+@Option+' ') like upper('% ' + N'NoDB'	+ N' %') Set @NoDB = 'True'
if upper(' '+@Option+' ') like upper('% ' + N'Only'	+ N' %') Set @Only = 'True'
if upper(' '+@Option+' ') like upper('% ' + N'Skip'	+ N' %') Set @Skip = 'True'
...
SELECT 
    ...
    ,dateadd(ms,[Duration]/1000,[StartTime]) as [DateTime_Stop]
	,N''profiler''		as [Object_name]
	,N''profiler''		as [Event]
	,N''' + @Version  + N'''					as	 [Package]
	,[CPU]				as [CPU]
	,[Duration]			as [Duration]
	,null				as [Page_server_reads]
	,null				as [Reads_physical]
	,[Reads]			as [Reads_logical]
	,[Writes]			as [Writes]
	,null				as [Spills]
	,null				as [Result_Code]
	,null				as [Result_Text]
	,null				as [Row_count]
	,null				as [Output_parameters]
	,[DatabaseID]		as [DatabaseID]
	,[LoginName]		as [LoginName]
	,[HostName]			as [HostName]
	,[SPID]				as [Session_id]
	,[NTUserName]		as [Nt_username]
	,null				as [Is_system]'
+case when @NoDB = 'True' 
			then N' ,null					as [Database_name]'
			else N'	,DB_NAME([DatabaseID])	as [Database_name]'
			end	+ N'
	,[ClientProcessID]	as [Client_pid]
	,[ApplicationName]	as [Client_app_name]
	,null				as [Event_sequence]
	,[TextData]			as [TextData]
	,HASHBYTES(''SHA2_256'', [TextData])	as	[Hash]
  FROM ' + @Source +  N'
  where [Duration] is not null 
	and [StartTime] is not null
	and [TextData] is not null
    and [Duration] <> 0
	and dateadd(ms,[Duration]/1000,[StartTime]) <  ''' + cast(@Stop as nvarchar(30)) + N''' '
 +' and dateadd(ms,[Duration]/1000,[StartTime]) >= ''' + cast(@Start as nvarchar(30)) + N''' '
 + N'order by [StartTime] '
...
-- ********** Далее идет копия куска из XE_Xel, 
--				кроме удаления архивов и записи в XEd_Source ********** --
...
execute [dbo].[XE_ExecLog]	 @Proc    = '[dbo].[XE_Merge]', @Caller = @Version
							,@Session = @Session
							,@Option  = @Option
							,@Steps   = @Steps
							,@Source  = '##tmp1'
...
if @List = 'True' 
	Print '***' + @Version +' ============ Update Dictionaries Completed ============'
...
select		 cast([l].[DateTime_Stop] as date)							as [Date]
		...
		into		#tmp3
		from		##tmp1		as [l]
		left join	[dbo].[XEd_Hash] as [Ha]	on   [Ha].[Hash] COLLATE DATABASE_DEFAULT = [l].[Hash] COLLATE DATABASE_DEFAULT 
