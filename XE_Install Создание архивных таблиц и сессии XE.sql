...
-- =============================================
	exec [dbo].[XE_Install] 
		 @Option	= 'replace /list /only /session Compression'	-- Опции выполнения
		,@Session	= 'sotivoli'  				-- сессия XE по умолчанию
		,@Source	= 
					  'C:\temp\XE_Log_*.xel'	-- файлы .xel	
--					  'C:\Документы\Проекты\Alidi\XE data\sql251\XE_Log_*.xel'	-- файлы .xel
...
Declare  @Version			as nvarchar(30)		= 'XE_Install v 5.2'
		,@Cmd				as nvarchar(max)
		,@List				as bit				= 'FALSE'
...
begin -- [XE_Sum]
	...
	CREATE TABLE [dbo].[XE_Sum] 
		...
		,[Reqs_Cnt]							[bigint]		NULL	-- requests
		,[Orders]							[bigint]		NULL	-- orders
		,[HashCount]						[bigint]		NULL	-- num unique Hash#
		,[OK]								[bigint]		NULL	-- nums Result = 'OK'
		,[Abort]							[bigint]		NULL	-- nums Result = 'Abort'
		,[Error]							[bigint]		NULL	-- nums Result = 'Error'
		 ) 
		on	[PRIMARY] 
end -- [XE_Sum]
...
***                                                                    ***
***   При необходимости миграции данных используйте процедуру         ***
