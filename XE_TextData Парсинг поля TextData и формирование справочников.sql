...
select	 [r].*
		,[r].[Date]					as [DateTime_Start]
		,[r].[Date]					as [DateTime_Stop]
		,[re].[Exec_Name]			as [Exec_Name]
		,[re].[Exec_Schema]			as [Exec_Schema]
		,[re].[Exec_DB]				as [Exec_DB]
		,[re].[Exec_Server]			as [Exec_Server]
		,[rt].[Table_Name]			as [Table_Name]
		,[rt].[Table_Schema]		as [Table_Schema]
		,[rt].[Table_DB]			as [Table_DB]
		,[rt].[Table_Server]		as [Table_Server]
	into		##tmp2
	from		#tmp2				as [r]
...
Drop table if exists #tmp1, #tmp2, #tmp2e, #tmp2t, #tmp3e, #tmp3t, ##tmp2 
