...
Declare	 @Version	nvarchar(30)	= N'XE_Convert v. 5.1'
		,@Cmd		nvarchar(max)
...
-- 0.3  Проверка сущуствования исходной таблицы
...
-- 0.4  Должна быть указана только одна опция выбора данных для конвертации:
...
if @List = 'TRUE' exec [dbo].[XE_PrintParam]	 @Caller		= @Version
												,@Session		= @Session
												,@Source		= @Source
												,@Table			= @Table
												,@Option		= @Option
...
--  0.7   Модифицируем @Option  для передачи в другие процедуры. Добавляем опцию 
...
print nchar(13) + N'*** ' + @Version + ' Information: Диапазон в исходной таблице: '
...
-- 2.2.2  Выводим информационное сообщение
	...
	print	 N'*** ' + @Version + ' Information: Обработка за дату: '
			+format(@Day, 'D', 'ru-ru')
...
--				импортируем все данные за день  из @Source
	...
	if @Table_Min_Time is null or @Table_Max_Time is null
		begin	-- Full Day
			...
			if @List = 'True' print	 N'*** ' + @Version + ' Information: Full day: '	
									+ cast(@Start as nvarchar(30))
...
--				импортируем нехватающие данные из @Source
	...
	and @Source_Min_Time < dateadd(ss, -@Uncertainty, @Table_Min_Time)
		begin	-- Start of Day
			...
			if @List = 'True' print	 N'*** ' + @Version + ' Information: <<<< of day: '	+ cast(@Start as nvarchar(30))
									+N' <= time < '		+ cast(@Stop  as nvarchar(30))
			if @Only = 'False' and @Profiler = 'True'
				exec XE_Import_Profile	 @Option	= @Option 
			 							,@Start		= @Start
...
--				импортируем нехватающие данные из @Source
	...
	and @Source_Max_Time > dateadd(ss, @Uncertainty, @Table_Max_Time)
		begin	-- End of Day
			...
			set @Stop	= cast(dateadd(day, 1, @Day) as datetime)
			if @List = 'True' print	 N'*** ' + @Version + ' Information: >>>>> of day: '	
									+ cast(@Start as nvarchar(30))
									+N' <= time < '		+ cast(@Stop  as nvarchar(30))
			if @Only = 'False' and @Profiler = 'True'
				exec XE_Import_Profile	 @Option	= @Option 
...
-- 2.3.4  Данные из целевой таблицы полностью перекрывают данные 
--			из исходной таблице по дню @Day: ничего не делаем

		if   @Source_Min_Time > dateadd(ss, -@Uncertainty, @Table_Min_Time)
		and  @Source_Max_Time < dateadd(ss,  @Uncertainty, @Table_Max_Time)
			print	 N'*** ' + @Version + ' Information: Данные за ' + cast(@Day as nvarchar(30)) 
...
-- ********** Step 3 завершающие операции ********** --

-- 3.1  Вызываем XE_TextData для обработки поля TextData из целевой таблицы @Table


if @Skip = 'False' and @List = 'True' and @Only = 'False'
	print '*** ' + @Version + ' Information: Обработка информации из полей [TextData]'
if @Skip = 'False' 	
	execute [dbo].[XE_ExecLog]	 @Proc		= N'[dbo].[XE_TextData]', @Caller = @Version
								,@Session	= @Session
								,@Option	= @Option

-- 3.2  Вызываем XE_TopSum 


if @Skip = 'False' and @List = 'True' and @Only = 'False'
	print '*** ' + @Version + ' Обработка XE_TopSum'
if @Skip = 'False' 	
