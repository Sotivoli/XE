...
,@Action		nvarchar(256)		= null	-- Действие
...
if @Action is not null begin -- @Action
	print replicate('*', len(@Prefix) + @L + @R)
	print @Prefix + @S + right(N'Тип действия (@Action) :' + coalesce(@Action,'<null>') , @R)
	...
	if upper(' ' + @Action + ' ') like N'% ' + upper('Session')	+ N' %'	print
		@Prefix + right(@F + N'Profiler'	,@L) + left(N': Уточнение  параметрова Session и Session#', @R)
	if upper(' ' + @Action + ' ') like N'% ' + upper('Table')	+ N' %'	print
		@Prefix + right(@F + N'Profiler'	,@L) + left(N': Уточнение  параметра Table', @R)
	if upper(' ' + @Action + ' ') like N'% ' + upper('Source')	+ N' %'	print
		@Prefix + right(@F + N'Profiler'	,@L) + left(N': Уточнение  параметра Source', @R)
	if upper(' ' + @Action + ' ') like N'% ' + upper('OrdersProc')	+ N' %'	print
		@Prefix + right(@F + N'Profiler'	,@L) + left(N': Уточнение  параметра OrdersProc', @R)
	if upper(' ' + @Action + ' ') like N'% ' + upper('Records')	+ N' %'	print
		@Prefix + right(@F + N'Profiler'	,@L) + left(N': Уточнение  параметров Records, Percenage, AddSum', @R)
	if upper(' ' + @Action + ' ') like N'% ' + upper('Object')	+ N' %'	print
		@Prefix + right(@F + N'Profiler'	,@L) + left(N': Уточнение  параметра Object', @R)
	if upper(' ' + @Action + ' ') like N'% ' + upper('Offset')	+ N' %'	print
		@Prefix + right(@F + N'Profiler'	,@L) + left(N': Уточнение  параметра Offset', @R)
